...
def get_date_optional(period_date_optional: str, date_format="%Y-%m-%d %H:%M:%S") -> [str, str]:
    ...
    logger.info(
        f"Функция {'get_date_optional'} принимает дату возвращает период за последние три месяца от вводимой даты"
    )
...
def spending_by_category(transactions: pd.DataFrame, category: str, date: Optional[str] = None) -> pd.DataFrame:
    """Функция принимает DataFrame, название категории и дату возвращает транзакции
    заданной категории за заданный период"""
    logger.debug(
        f"Выполняется функция которая принимает DataFrame, название категории и "
        f"дату возвращает транзакции заданной категории за заданный период: {'spending_by_category'}"
    )
    ...
    filtered_category_transactions = category_transactions.loc[
        (pd.to_datetime(category_transactions["Дата операции"], dayfirst=True) >= start_pd)
        & (pd.to_datetime(category_transactions["Дата операции"], dayfirst=True) <= end_pd)
    ]
