...
def test_get_card_with_spend():
    mock_get_card_with_spend = Mock(
        return_value=[{'Yes': "52", 'No': "2"}]
    )
...
def test_get_currency(mock_get):
    ...
    assert get_currency("../data/user_settings.json") == [{'currency': 'USD', 'rate': '78.5'}, {'currency': 'EUR', 'rate': '91.0'}]
...
def test_get_stock(mock_get):
    ...
    assert get_stock("../data/user_settings_1.json") == [{'price': '213.8800', 'stock': 'AAPL'}]
    stock = 'AAPL'
    mock_get.assert_called_with(f"https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={stock}&apikey={API_KEY_STOCK}")
