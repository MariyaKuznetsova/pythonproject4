...
import pandas as pd
from src.services import excel_func


def test_excel_func():
    mock_excel_func = Mock(return_value={"Yes": [50, 21], "No": [131, 2]})
...
def test_excel_func_1():
    mock_excel_func_1 = Mock(return_value={})
...
def test_excel_func_2(mock_excel_file):
    ...
    assert excel_func("../data/operation_test.xlsx") == [
        {"Валюта операции": "RUB", "Номер карты": "*7197", "Статус": "OK"}
    ]
...
def test_simple_search():
    mock_simple_search = Mock(return_value=[{"Yes": 4, "No": 6}])
...
def test_simple_search_1():
    mock_simple_search_1 = Mock(return_value=[{}])
